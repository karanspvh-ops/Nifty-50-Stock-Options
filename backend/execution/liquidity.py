...
Three checks in order:
  ...
  1. Open interest     — must have >= MIN_OI_LOTS of open interest (in lots, not units)
  2. Bid-ask spread    — spread must be <= MAX_SPREAD_PCT of mid price
...
def has_liquidity(opt_token: str, opt_symbol: str, lot_size: int,
    ...
    Checks (in order): ban list, open interest, bid-ask spread.
    ...
    # ── 1. Open interest (in lots) + bid-ask spread ───────────────────────────
    try:
