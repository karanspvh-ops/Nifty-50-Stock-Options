"""tradable_tracker.py — Watches every stock and records tradable windows.
...
Active windows are held in memory for the dashboard's flashing tradable panel.
Closed windows are persisted to SQLite and rolled up into a daily report.

Runs as a background thread, every SCAN_INTERVAL seconds.

Edit this file to change:
  - Which stocks are evaluated (ranked / MAX_EVALUATE cap)
  - What data is stored in each tradable window
...
from backend.universe.scanner      import get_stocks_for_index, get_meta
...
MAX_EVALUATE    = 80
MIN_ABS_MOVE    = 0.3
REPORTS_DIR     = os.path.join(_ROOT, "reports", "tradable")
...
class TradableTracker:
    ...
    def __init__(self):
        ...
        self._active: Dict[str, dict] = {}   # token → active window
    ...
    # ── One sweep over the F&O universe ───────────────────────────────────────
    ...
    def _sweep(self):
        ...
        stocks       = get_stocks_for_index("FNO", fno_only=True)
    ...
    def mark_traded(self, symbol: str):
        """Called by trading engine when it enters a trade on a symbol."""
        for w in self._active.values():
            if w["symbol"] == symbol:
                w["_was_traded"] = True
